### 声明: 本仓库主要是为了传播编程知识, 感谢以下书籍作者, 如果侵犯您的权益, 诚恳的向您表示歉意, 同时我会删除这些书籍
### Statement: Warehouse is mainly to spread programming knowledge, Thanks to the author of the following books, If I infringe on your interests, Sincerely apologize to you and I will delete these books.

### 建议读者浏览器安装octotree插件, 方便查找文件, 插件详细信息请参阅https://github.com/ovity/octotree

## Table of content (目录仅作展示,链接暂未提供)


<!-- vim-markdown-toc GFM -->

* [Blog](#blog)
	* [JDK之ThreadPoolExecutor源码分析.pdf](#jdk之threadpoolexecutor源码分析pdf)
	* [refactor.pdf](#refactorpdf)
	* [线程池增加监控.pdf](#线程池增加监控pdf)
	* [自定义实现Java线程池1-模拟jdk线程池执行流程.pdf](#自定义实现java线程池1-模拟jdk线程池执行流程pdf)
	* [自定义实现Java线程池2-完善异常处理和去除同步.pdf](#自定义实现java线程池2-完善异常处理和去除同步pdf)
	* [计算多线程环境下执行任务时间.pdf](#计算多线程环境下执行任务时间pdf)
* [Chinese books](#chinese-books)
	* [DB](#db)
		* [高性能MySql第三版.pdf](#高性能mysql第三版pdf)
	* [Design Patterns](#design-patterns)
		* [Head First设计模式.pdf](#head-first设计模式pdf)
		* [设计模式_可复用面向对象软件的基础.pdf](#设计模式_可复用面向对象软件的基础pdf)
		* [设计模式之禅（第2版）.pdf](#设计模式之禅第2版pdf)
	* [Editor](#editor)
		* [LaTeX入门.pdf](#latex入门pdf)
		* [vim_user_manual_603.0.pdf](#vim_user_manual_6030pdf)
		* [Vim实用技巧.pdf](#vim实用技巧pdf)
	* [Google](#google)
		* [Google-Bigtable_1.0.pdf](#google-bigtable_10pdf)
		* [Google-File-System_1.0.pdf](#google-file-system_10pdf)
		* [Google-MapReduce_1.0.pdf](#google-mapreduce_10pdf)
	* [Java](#java)
		* [Effective Java中文版（第2版）.pdf](#effective-java中文版第2版pdf)
		* [Elasticsearch 权威指南（中文版）.pdf](#elasticsearch-权威指南中文版pdf)
		* [How Tomcat Works.pdf](#how-tomcat-workspdf)
		* [Java 8 - GC cheatsheet.pdf](#java-8---gc-cheatsheetpdf)
		* [Java 8实战.pdf](#java-8实战pdf)
		* [Java concurrent in practice.pdf](#java-concurrent-in-practicepdf)
		* [Java多线程编程核心技术.pdf](#java多线程编程核心技术pdf)
		* [Java并发编程实战.pdf](#java并发编程实战pdf)
		* [Java并发编程的艺术.pdf](#java并发编程的艺术pdf)
		* [Java编程思想(第4版)（带目录书签）.pdf](#java编程思想第4版带目录书签pdf)
		* [Java虚拟机规范（JavaSE7）.pdf](#java虚拟机规范javase7pdf)
		* [JAVA解惑.pdf](#java解惑pdf)
		* [JSR-133 Java TM 内存模型与线程规范.pdf](#jsr-133-java-tm-内存模型与线程规范pdf)
		* [Netty实战.pdf](#netty实战pdf)
		* [Spring Cloud微服务实战.pdf](#spring-cloud微服务实战pdf)
		* [Spring实战（第4版）.pdf](#spring实战第4版pdf)
		* [Spring技术内幕（第二版）.pdf](#spring技术内幕第二版pdf)
		* [Spring揭秘.pdf](#spring揭秘pdf)
		* [The Java Language Specification Java SE 8 Edition.pdf](#the-java-language-specification-java-se-8-editionpdf)
		* [大型网站技术架构：核心原理与案例分析+李智慧.pdf](#大型网站技术架构核心原理与案例分析李智慧pdf)
		* [深入理解Java虚拟机：JVM高级特性与最佳实践.pdf](#深入理解java虚拟机jvm高级特性与最佳实践pdf)
		* [精通Java并发编程（第2版）.pdf](#精通java并发编程第2版pdf)
		* [重构：改善既有代码的设计.pdf](#重构改善既有代码的设计pdf)
		* [阿里巴巴Java开发手册（详尽版）.pdf](#阿里巴巴java开发手册详尽版pdf)
	* [Kafka](#kafka)
		* [Kafka权威指南.pdf](#kafka权威指南pdf)
	* [Linux](#linux)
		* [vbird-linux-basic-4e.pdf](#vbird-linux-basic-4epdf)
		* [鸟哥的Linux私房菜-基础篇.第四版.pdf](#鸟哥的linux私房菜-基础篇第四版pdf)
	* [Redis](#redis)
		* [Redis开发与运维.pdf](#redis开发与运维pdf)
* [English books](#english-books)
	* [Algorithhms](#algorithhms)
		* [Algorithhms 4th.pdf](#algorithhms-4thpdf)
	* [Common](#common)
		* [code-complete-2nd-edition-v413hav.pdf](#code-complete-2nd-edition-v413havpdf)
	* [DB](#db-1)
		* [MySQL_High_Availability.pdf](#mysql_high_availabilitypdf)
	* [Design Patterns](#design-patterns-1)
		* [Design Patterns, Elements of Reusable Object-Oriented Software.pdf](#design-patterns-elements-of-reusable-object-oriented-softwarepdf)
		* [DesignPatternsCard.pdf](#designpatternscardpdf)
		* [Head First Design Patterns.pdf](#head-first-design-patternspdf)
	* [Editor](#editor-1)
		* [emacs.pdf](#emacspdf)
		* [Practical.Vim.2nd.Edition.2015.10.pdf](#practicalvim2ndedition201510pdf)
	* [Google](#google-1)
		* [Bigtable_A Distributed Storage System for Structured Data.pdf](#bigtable_a-distributed-storage-system-for-structured-datapdf)
		* [MapReduce_Simplified Data Processing on Large Clusters.pdf](#mapreduce_simplified-data-processing-on-large-clusterspdf)
		* [The Google File System.pdf](#the-google-file-systempdf)
	* [Java](#java-1)
		* [Computer Systems - A Programmer's Perspective.pdf](#computer-systems---a-programmers-perspectivepdf)
		* [Concurrent programming in java design principles and pattern.pdf](#concurrent-programming-in-java-design-principles-and-patternpdf)
		* [Effective Java 1st Edition.pdf](#effective-java-1st-editionpdf)
		* [Effective Java 3rd Edition.pdf](#effective-java-3rd-editionpdf)
		* [Effective.Java.3rd.Edition.2018.1.pdf](#effectivejava3rdedition20181pdf)
		* [How Tomcat Works.pdf](#how-tomcat-workspdf-1)
		* [Java 8 - GC cheatsheet.pdf](#java-8---gc-cheatsheetpdf-1)
		* [Java 8 in Action.pdf](#java-8-in-actionpdf)
		* [Java concurrency in practice.pdf](#java-concurrency-in-practicepdf)
		* [Java Puzzlers.pdf](#java-puzzlerspdf)
		* [JSR-133 Java TM Memory Model and Thread Specification.pdf](#jsr-133-java-tm-memory-model-and-thread-specificationpdf)
		* [Netty in Action.pdf](#netty-in-actionpdf)
		* [Spring Boot in Action.pdf](#spring-boot-in-actionpdf)
		* [Spring Microservices in Action.pdf](#spring-microservices-in-actionpdf)
		* [The Java Language Specification Java SE 8 Edition.pdf](#the-java-language-specification-java-se-8-editionpdf-1)
		* [The Java Virtual Machine Specification, Java SE 8 Edition.pdf](#the-java-virtual-machine-specification-java-se-8-editionpdf)
		* [Thinking In Java 4th Edition.pdf](#thinking-in-java-4th-editionpdf)
	* [Kafka](#kafka-1)
		* [Kafka The Definitive Guide.pdf](#kafka-the-definitive-guidepdf)
* [PDF Reader](#pdf-reader)
	* [SumatraPDF-3.1.2-64.zip](#sumatrapdf-312-64zip)

<!-- vim-markdown-toc -->

<!--
## Blog

### JDK之ThreadPoolExecutor源码分析.pdf
### refactor.pdf
### 线程池增加监控.pdf
### 自定义实现Java线程池1-模拟jdk线程池执行流程.pdf
### 自定义实现Java线程池2-完善异常处理和去除同步.pdf
### 计算多线程环境下执行任务时间.pdf

## Chinese books

### DB

#### 高性能MySql第三版.pdf

### Design Patterns

#### Head First设计模式.pdf
#### 设计模式_可复用面向对象软件的基础.pdf
#### 设计模式之禅（第2版）.pdf

### Editor

#### LaTeX入门.pdf
#### vim_user_manual_603.0.pdf
#### Vim实用技巧.pdf

### Google

#### Google-Bigtable_1.0.pdf
#### Google-File-System_1.0.pdf
#### Google-MapReduce_1.0.pdf

### Java

#### Effective Java中文版（第2版）.pdf
#### Elasticsearch 权威指南（中文版）.pdf
#### How Tomcat Works.pdf
#### Java 8 - GC cheatsheet.pdf
#### Java 8实战.pdf
#### Java concurrent in practice.pdf
#### Java多线程编程核心技术.pdf
#### Java并发编程实战.pdf
#### Java并发编程的艺术.pdf
#### Java编程思想(第4版)（带目录书签）.pdf
#### Java虚拟机规范（JavaSE7）.pdf
#### JAVA解惑.pdf
#### JSR-133 Java TM 内存模型与线程规范.pdf
#### Netty实战.pdf
#### Spring Cloud微服务实战.pdf
#### Spring实战（第4版）.pdf
#### Spring技术内幕（第二版）.pdf
#### Spring揭秘.pdf
#### The Java Language Specification Java SE 8 Edition.pdf
#### 大型网站技术架构：核心原理与案例分析+李智慧.pdf
#### 深入理解Java虚拟机：JVM高级特性与最佳实践.pdf
#### 精通Java并发编程（第2版）.pdf
#### 重构：改善既有代码的设计.pdf
#### 阿里巴巴Java开发手册（详尽版）.pdf

### Kafka

#### Kafka权威指南.pdf

### Linux

#### vbird-linux-basic-4e.pdf
#### 鸟哥的Linux私房菜-基础篇.第四版.pdf

### Redis

#### Redis开发与运维.pdf


## English books

### Algorithhms

#### Algorithhms 4th.pdf

### Common

#### code-complete-2nd-edition-v413hav.pdf

### DB

#### MySQL_High_Availability.pdf

### Design Patterns

#### Design Patterns, Elements of Reusable Object-Oriented Software.pdf
#### DesignPatternsCard.pdf
#### Head First Design Patterns.pdf

### Editor

#### emacs.pdf
#### Practical.Vim.2nd.Edition.2015.10.pdf

### Google

#### Bigtable_A Distributed Storage System for Structured Data.pdf
#### MapReduce_Simplified Data Processing on Large Clusters.pdf
#### The Google File System.pdf

### Java

#### Computer Systems - A Programmer's Perspective.pdf
#### Concurrent programming in java design principles and pattern.pdf
#### Effective Java 1st Edition.pdf
#### Effective Java 3rd Edition.pdf
#### Effective.Java.3rd.Edition.2018.1.pdf
#### How Tomcat Works.pdf
#### Java 8 - GC cheatsheet.pdf
#### Java 8 in Action.pdf
#### Java concurrency in practice.pdf
#### Java Puzzlers.pdf
#### JSR-133 Java TM Memory Model and Thread Specification.pdf
#### Netty in Action.pdf
#### Spring Boot in Action.pdf
#### Spring Microservices in Action.pdf
#### The Java Language Specification Java SE 8 Edition.pdf
#### The Java Virtual Machine Specification, Java SE 8 Edition.pdf
#### Thinking In Java 4th Edition.pdf

### Kafka

#### Kafka The Definitive Guide.pdf


## PDF Reader

### SumatraPDF-3.1.2-64.zip
-->
